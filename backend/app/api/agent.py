    ...
    try:
        ...
        while response.stop_reason == "tool_use":
            ...
            for tool_use in tool_uses:
                                    ...
                                    actions_taken.append({"tool": tool_use.name, "input": tool_use.input, "result": json.loads(result)})
...
async def get_history(
            ...
            result = await db.execute(
                select(AgentChat).where(AgentChat.client_id == current_user.id).order_by(AgentChat.timestamp.desc()).limit(limit)
)
    chats = result.scalars().all()
    return [{"id": str(c.id), "role": c.role.value, "message": c.message, "actions_taken": c.actions_taken, "timestamp": c.timestamp.isoformat()} for c in reversed(chats)]
